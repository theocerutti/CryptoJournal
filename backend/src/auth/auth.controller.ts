...
export class AuthController {
  ...
  async login(
    ...
    const refreshToken = await this.refreshTokenService.generateRefreshToken(
      info.user,
      EXPIRATION_REFRESH_TOKEN
    ); // TODO: generateRefreshToken from refreshToken in DB?
    return AuthController.buildResponsePayload(
      info.user,
      info.token,
      refreshToken
    );
  ...
  public async refresh(@Body() body: RefreshRequest): Promise<string> {
    return await this.refreshTokenService.createAccessTokenFromRefreshToken(
      body.refresh_token
    );
  }
