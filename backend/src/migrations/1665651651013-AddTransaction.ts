...
export class AddTransaction1665651651013 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "user" ADD "erc20Address" character varying`
    );
    await queryRunner.query(
      `ALTER TABLE "user" ADD "btcAddress" character varying`
    );
    await queryRunner.query(
      `CREATE TYPE "public"."investment_orderstatus_enum" AS ENUM('OPEN', 'CLOSED')`
    );
    await queryRunner.query(
      `ALTER TABLE "investment" ADD "orderStatus" "public"."investment_orderstatus_enum" NOT NULL DEFAULT 'OPEN'`
    );
    await queryRunner.query(
      `CREATE TYPE "public"."investment_type_enum" AS ENUM('NONE', 'GIFT')`
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "investment" DROP COLUMN "orderStatus"`
    );
